...
public class HotelsDAOImpl implements HotelsDAO {
    ...
    @Nullable private List<Room> rooms = new ArrayList<>(0);

    public HotelsDAOImpl(ArrayList<Room> roomsArray) {
        rooms.addAll(roomsArray);
    }

    public ArrayList<Room> findRooms(int price, int persons, String city, String hotel) {
        if (rooms == null || rooms.size() == 0 || city == null || hotel == null) {return new ArrayList<Room>(0);}
